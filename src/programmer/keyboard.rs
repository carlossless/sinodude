...
impl Keyboard {
    pub fn new() -> Self {
        ...
        let kb_device_info = api
            .device_list()
            .find(|d| d.vendor_id() == 0x5ac && d.product_id() == 0x024f);

        if let Some(device_info) = kb_device_info {
            Self::reset_device(&api, &device_info);
        } else {
    ...
    fn reset_device(api: &HidApi, device_info: &DeviceInfo) {
        println!("Found KB. Resetting into bootloader...");
        let device = device_info.open_device(&api).unwrap();
        let buf_reset: [u8; 6] = [0x05, 0x75, 0x00, 0x00, 0x00, 0x00];
        let _ = device.send_feature_report(&buf_reset); // ignore errors, a reset will happen immediately
        println!("Waiting for bootloader device...");
        thread::sleep(time::Duration::from_millis(2000));
    }

    pub fn read_cycle(&self) {
